...
void show_single_pos(int ref_pos, char ref_base, char cons_base, BaseCountsP bcs) {
	printf("%d %c %c %d %d %d %d %d %d %d %d %d %d\n", 
	       ref_pos, 
	       ref_base, 
	       cons_base,
	       bcs->cov, 
...
void print_region(MapAlignmentP maln, int reg_start, int reg_end,
	...
	/* Alloc memories for the string to hold each read (plus .'s outside)
	 ...
	 plus the code for whether it's truncated and reverse complemented */
	read_str = (char*)save_malloc(strlen(aln_ref) * sizeof(char) + 1);
	read_id = (char*)save_malloc((MAX_ID_LEN + 2) * sizeof(char) + 1);
...
int merge_pwaln_into_maln(PWAlnFragP pwaln, MapAlignmentP maln) {
  ...
  char* ins_seq;
  AlnSeqP asp;
  int this_ref_gaps[INIT_ALN_SEQ_LEN + 1];
